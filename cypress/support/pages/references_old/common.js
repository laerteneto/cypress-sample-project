...
export const assertPageContent = page => {
  switch (page) {
    case 'welcome':
      cy.get('h1').contains('Welcome to Customer App')
      cy.contains('Please provide your name:')
      cy.get(selectors.usernameInput).should('have.attr', 'type', 'text')
      cy.get(selectors.submitBtn)
        .should('have.attr', 'type', 'button')
        .and('value', 'Submit')
      break
    case 'customerList':
      cy.get(selectors.headerTitle).contains('Welcome to Customer App')

      cy.get(selectors.table)
        .find(selectors.tableContentList)
        .children()
        .should($buttonsList => {
